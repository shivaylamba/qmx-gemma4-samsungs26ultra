...
static void qmx_tracking_log_callback(
    ...
    if (std::strstr(text, "kleidiai: primary q8 kernel feature SME") != nullptr) {
        g_q8_sme_kernel.store(true);
    }
    if (std::strstr(text, "kleidiai: SME enabled") != nullptr) {
        g_sme_enabled.store(true);
    }
...
Java_com_arm_aichat_internal_InferenceEngineImpl_load(JNIEnv *env, jobject, jstring jmodel_path) {
    ...
    qmx_blocks << ")\\.";
    const std::string qmx_blocks_pattern = qmx_blocks.str();
...
static llama_context *init_context(
    ...
    const int automatic_threads = std::max(N_THREADS_MIN, std::min(N_THREADS_MAX,
            (int) sysconf(_SC_NPROCESSORS_ONLN) - N_THREADS_HEADROOM));
    const int n_threads = requested_threads > 0 ? requested_threads : automatic_threads;
...
Java_com_arm_aichat_internal_InferenceEngineImpl_benchModel(JNIEnv *env, jobject /*unused*/, jint pp, jint tg,
                                                      jint pl, jint nr, jint threads) {
    auto *context = init_context(g_model, pp, threads);
    if (!context) {
